...
#       2) Items table with totals and VAT (centered)
#       3) Payment schedule table (centered)
#       4) Bank details in a 4-column layout (based on selected bank)
#       5) Terms & Conditions and final message
#     """
#     pdf = init_pdf()
#     if not pdf:
...
#         # --- 4) BANK DETAILS (4 Columns) ---
#         print_bank_details(pdf, invoice_data.get("bank", "BNP Paribas"))
...
# # --- Bank Selection ---
# st.subheader("Bank Transfer Details Selection")
# bank_choice = st.selectbox("Select your bank for transfer", ["BNP Paribas", "Societe Generale"])
...
# PDF Initialization
# -------------------------------------------------------------------
def init_pdf(base_font_size=10) -> FPDF:
    """Initialize FPDF and register Bookman fonts using a specified base font size."""
    pdf = FPDF()
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)
    try:
        pdf.add_font("Bookman", "", FONT_BOOKMAN_REGULAR, uni=True)
        pdf.add_font("Bookman", "B", FONT_BOOKMAN_BOLD, uni=True)
        pdf.set_font("Bookman", size=base_font_size)
        return pdf
    except Exception as e:
        st.error(f"PDF initialization failed: {e}")
        return None
...
# -------------------------------------------------------------------
# Core PDF Generation Function (returns an FPDF instance)
# -------------------------------------------------------------------
def create_invoice_pdf_instance(invoice_data: dict, base_font_size: int) -> FPDF:
    pdf = init_pdf(base_font_size)
    if not pdf:
        return None
    ...
    try:
        ...
        col_perc_w = 40
        col_amt_w = 40
        pay_table_width = col_payname_w + col_paydate_w + col_perc_w + col_amt_w
        pay_table_left = 10 + (190 - pay_table_width) / 2
...
# -------------------------------------------------------------------
# PDF Generation Wrapper with Font Size Adjustment
# -------------------------------------------------------------------
def create_invoice_pdf(invoice_data: dict, initial_font_size: int = 10) -> bytes:
    max_attempts = 5
    attempt = 0
    ...
    while attempt < max_attempts:
        pdf_instance = create_invoice_pdf_instance(invoice_data, font_size)
        if pdf_instance is None:
            break
        num_pages = pdf_instance.page_no()
        if num_pages == 1:
            pdf_bytes = bytes(pdf_instance.output(dest="S"))
            break
        else:
            # Decrease the base font size and retry
            font_size -= 1
            attempt += 1

    if pdf_bytes is None:
        st.error("Could not fit the invoice on one page with the available adjustments.")
    return pdf_bytes
...
if st.button("Generate Invoice"):
    ...
    if validation_errors:
        ...
        pdf_bytes = create_invoice_pdf(invoice_data, initial_font_size=10)
        st.session_state.pdf_data = {
            "bytes": pdf_bytes,
            "invoice_number": invoice_number
        } if pdf_bytes else None
