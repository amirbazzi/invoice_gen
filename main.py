...
# FONT_REGULAR = os.path.join(BASE_DIR, "DejaVuSansCondensed.ttf")

# print(f"DEBUG FONT_REGULAR DIR =================== {FONT_REGULAR}")

# FONT_BOLD = os.path.join(BASE_DIR, "DejaVuSansCondensed-Bold.ttf")
...
def ordinal(n: int) -> str:
    """0-based index to ordinal word: first, second, etc."""
    ordinals = ["first", "second", "third", "fourth", "fifth", 
                "sixth", "seventh", "eighth", "ninth", "tenth"]
    return ordinals[n] if n < len(ordinals) else f"{n+1}th"

def init_pdf() -> FPDF:
    """Initialize FPDF and register both DejaVu and Bookman fonts."""
    pdf_invoice = FPDF()
    pdf_invoice.add_page()
    pdf_invoice.set_auto_page_break(auto=True, margin=15)
    ...
    # Check and display font paths + existence
    st.write("FONT_BOOKMAN_REGULAR path:", FONT_BOOKMAN_REGULAR)
    st.write("Exists?", os.path.exists(FONT_BOOKMAN_REGULAR))

    st.write("FONT_BOOKMAN_BOLD path:", FONT_BOOKMAN_BOLD)
...
def create_invoice_pdf(invoice_data: dict) -> bytes:
    """
    Creates a PDF with:
      1) Header (logo, co. info, invoice #, date, client info)
      2) Items table: 3 columns (DESC | PRICE | TO BE PAID),
         sum in 2nd & 3rd columns, VAT & final under 3rd column
      3) Payment Schedule (3 columns: Payment, Date, % or Amt)
    ...
    try:
        ...
        def example_bank_details_4columns(pdf):
            pdf.set_font("Bookman", "B", 10)
            pdf.cell(0, 5, "Bank Transfer Details:", ln=1)
            pdf.ln(2)
...
# 3) Payment Schedule (Suggested: 50%, 30%, 20%)
...
for i in range(int(num_payments)):
    with st.expander(f"Payment {i+1}", expanded=(i < 3)):
        ...
        if date_enabled:
            pay_date = st.date_input(f"Date {i+1}", value=date.today(), key=f"pay_date_{i}")
        else:
            pay_date = ""
